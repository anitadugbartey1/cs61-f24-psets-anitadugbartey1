...
struct io61_file {
    ...
    std::recursive_mutex rm;  // a single mutex per file
    std::mutex m;
...
int io61_readc(io61_file* f) {
    ...
    if (f->pos_tag == f->end_tag) {
        io61_fill(f);
        if (f->pos_tag == f->end_tag) return -1;
    }
...
ssize_t io61_read(io61_file* f, unsigned char* buf, size_t sz) {
    ...
    while (nread != sz) {
        if (f->pos_tag == f->end_tag) {
            int r = io61_fill(f);
            if (r == -1 && nread == 0) return -1;
            else if (f->pos_tag == f->end_tag) break;
        }
...
ssize_t io61_write(io61_file* f, const unsigned char* buf, size_t sz) {
    std::unique_lock<std::recursive_mutex> lock(f->rm);
    assert(!f->positioned);
    size_t nwritten = 0;
    while (nwritten != sz) {
        if (f->end_tag == f->tag + f->cbufsz) {
            int r = io61_flush(f);
            if (r == -1 && nwritten == 0) return -1;
            else if (r == -1) break;
        }
        size_t nleft = f->tag + f->cbufsz - f->end_tag;
        size_t ncopy = std::min(sz - nwritten, nleft);
        memcpy(&f->cbuf[f->end_tag - f->tag], &buf[nwritten], ncopy);
        nwritten += ncopy;
        f->end_tag += ncopy;
        f->dirty = true;
    }
...
static int io61_flush_dirty(io61_file* f) {
    ...
    while (flush_tag != f->end_tag) {
        ssize_t nw = write(f->fd, &f->cbuf[flush_tag - f->tag], f->end_tag - flush_tag);
        if (nw >= 0) flush_tag += nw;
        else if (errno != EINTR && errno != EINVAL) return -1;
    }
    f->dirty = false;
...
static int io61_flush_dirty_positioned(io61_file* f) {
    ...
    off_t flush_tag = f->tag;
    while (flush_tag != f->end_tag) {
        ssize_t nw = pwrite(f->fd, &f->cbuf[flush_tag - f->tag], f->end_tag - flush_tag, flush_tag);
        if (nw >= 0) flush_tag += nw;
        else if (errno != EINTR && errno != EINVAL) return -1;
    }
    f->dirty = false;
...
static int io61_flush_clean(io61_file* f) {
    ...
    if (!f->positioned && f->seekable) {
        if (lseek(f->fd, f->pos_tag, SEEK_SET) == -1) return -1;
...
ssize_t io61_pread(io61_file* f, unsigned char* buf, size_t sz, off_t off) {
    ...
    if (!f->positioned || off < f->tag || off >= f->end_tag) {
        if (io61_pfill(f, off) == -1) return -1;
    }
...
static int io61_pfill(io61_file* f, off_t off) {
    ...
    if (f->dirty && io61_flush(f) == -1) return -1;
    off = off - (off % 8192);
...
int io61_try_lock(io61_file* f, off_t off, off_t len, int locktype) {
    ...
    if (may_overlap_with_other_lock(f, off, len)) return -1;
