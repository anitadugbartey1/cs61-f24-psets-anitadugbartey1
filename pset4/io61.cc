...
struct io61_file {
   ...
   // Double buffering for reads (helps with reverse reads and seeking)
   unsigned char second_buf[BUFFER_SIZE];
   size_t second_buf_size;
   size_t second_buf_pos;
   off_t second_tag_position;
...
static int io61_fill_buffer(io61_file* f) {
   ...
   if (f->reverse_mode) {
       ...
       off_t target_pos = f->pos - BUFFER_SIZE;
       if (target_pos < 0) target_pos = 0;
       
       // Use double buffering for reverse reads
       std::swap(f->cbuf, f->second_buf);
       std::swap(f->cbuf_size, f->second_buf_size);
       std::swap(f->cbuf_pos, f->second_buf_pos);
       std::swap(f->tag_position, f->second_tag_position);
       
       off_t seek_result = lseek(f->fd, target_pos, SEEK_SET);
       if (seek_result == -1) return -1;
       
       ssize_t nr = read(f->fd, f->cbuf, BUFFER_SIZE);
       if (nr <= 0) return nr;
   ...
   // If we know file size, don't read past it
   if (f->size_known && f->pos + to_read > f->file_size) {
       to_read = f->file_size - f->pos;
   }
   
   f->cbuf_size = read(f->fd, f->cbuf, to_read);
   if (f->cbuf_size > 0) {
...
int io61_readc(io61_file* f) {
   // If buffer is empty or we've read all buffered data
   if (f->reverse_mode) {
       if (f->cbuf_pos >= f->cbuf_size) {
           ssize_t nr = io61_fill_buffer(f);
           if (nr <= 0) return -1;
       }
       unsigned char ch = f->cbuf[f->cbuf_pos--];
       f->pos--;
       return ch;
   } else {
       if (f->cbuf_pos >= f->cbuf_size) {
...
ssize_t io61_read(io61_file* f, unsigned char* buf, size_t sz) {
   ...
   if (sz >= BUFFER_SIZE * 4) {  // Threshold for direct reads
...
int io61_flush(io61_file* f) {
   if (f->mode != O_RDONLY && f->cbuf_pos > 0) {
       // If current position doesn't match buffer position, seek first
...
int io61_seek(io61_file* f, off_t off) {
   ...
   // Check if seek is within current buffer
   ...
   // Check second buffer
   off_t second_start = f->second_tag_position - f->second_buf_size;
   off_t second_end = f->second_tag_position;
   if (off >= second_start && off < second_end) {
       // Swap buffers if target is in second buffer
       std::swap(f->cbuf, f->second_buf);
       std::swap(f->cbuf_size, f->second_buf_size);
       std::swap(f->cbuf_pos, f->second_buf_pos);
       std::swap(f->tag_position, f->second_tag_position);
       f->cbuf_pos = off - second_start;
       f->pos = off;
       f->reverse_mode = false;
       return 0;
   }
   
   // Set reverse mode based on seek direction
   f->reverse_mode = (off < f->pos);
   
   // Invalidate buffers
   f->cbuf_size = f->cbuf_pos = 0;
   f->second_buf_size = f->second_buf_pos = 0;
   f->pos = off;
   
   return 0;
}

// Helper functions to check for errors during write operations and to synchronize file times
